...
describe("runInit", () => {
  ...
  it("projectId 未指定なら getProject で解決する", async () => {
    const adapter = { getStatusMap: vi.fn().mockResolvedValue({ open: 1, in_progress: 2, resolved: 3, closed: 4 }) } as any;
    const rest = { getMyself: vi.fn().mockResolvedValue({ id: 5, name: "me" }), getProject: vi.fn().mockResolvedValue({ id: 99, projectKey: "PROJ", name: "P" }) } as any;
    const out = await runInit({ cwd: dir, projectKey: "PROJ" }, { adapter, rest });
    expect(rest.getProject).toHaveBeenCalledWith("PROJ");
    const written = JSON.parse(readFileSync(join(dir, ".claude", "backlog-agent-sync", "project.json"), "utf8"));
    expect(written.projectId).toBe(99);
    expect(out.ok).toBe(true);
  });
});
