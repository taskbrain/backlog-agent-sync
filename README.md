...
"fieldRules": {
  ...
  "summarize": "off"                   // "claude" でターン要約に LLM 1段落要約を追加（opt-in）
}
```

- キーワードで決められない高度な判断（真の優先度・適切な担当者・カテゴリ）は、エージェント側が MCP `update_issue` で上書きします（skills/backlog-tracking のポリシー）。
- **`summarize: "claude"` の注意**: ターン要約の生成に `claude -p --bare` を呼ぶため **`ANTHROPIC_API_KEY` が必須**で、**API クレジットを消費**します。既定は off（`last_assistant_message` がそのまま記録されるため、通常は不要です）。
...
| `UserPromptSubmit` | このターンのプロンプトを記録（課題説明・ターン要約の素材）し、ターン開始時の HEAD SHA を控える。フィールド自動設定（fieldRules）の入力にも使う |
